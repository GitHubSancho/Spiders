    ...
    def re_load_conf(self, last_loading_time, refresh_time=300):
        if time.time() - last_loading_time > refresh_time:  # 每隔一段时间读取配置信息
            with ThreadPoolExecutor() as thread_pool:
                # REVIEW:
                conf = thread_pool.submit(self.load_conf).result()
                hubs = thread_pool.submit(self.load_hubs).result()
...
class Crawler:
    ...
    def _save_to_db(self, urls, mode):
        ...
        # REVIEW:
        with ThreadPoolExecutor() as thread_pool:
            thread_pool.submit(self.mongo.insert_many, documents)
    ...
    def crawl(self):
        ...
        for _i in range(20):
            # REVIEW:
            with ThreadPoolExecutor(self.MAX_WORKERS_THREAD) as thread_pool:
                print(_i)
                # while 1:
                last_loading_time, conf, hubs = self.loader.re_load_conf(
                    last_loading_time, 300)  # 刷新配置文件
                self._ischange(conf, hubs)  # 判断是否需要刷新
                tasks = thread_pool.submit(self.mongo.get,self.MAX_WORKERS_CONCURRENT,
                                       self.conf['pending_threshold']).result()  # 获取链接
                with requests.session() as session:
                    for task in tasks:  # 遍历待下载网址
                        # 抓取网页
                        # NOTE:960  504462631.0 525481.9     64.1
                        status, html, redirected_url = thread_pool.submit(
                            self.downloader.fetch, session,
                            task['url']).result()
                        # 更新状态
                        # NOTE: 960  108409110.0 112926.2     13.8
                        thread_pool.submit(self.mongo._set_status, status,
                                           self.parser._zip_html(html),
                                           redirected_url, task['mode'],
                                           task['failure'],
                                           self.conf['failure_threshold'])
                        # 解析数据
                        # 960   27407994.0  28550.0      3.5
                        links = self.parser.process(status, html,
                                                    redirected_url,
                                                    task['mode'], task['host'])
                        # 添加数据
