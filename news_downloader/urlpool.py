...
class UrlPool:
    ...
    用于管理url的网址池
    '''
    def __init__(self, db_name, set_name, host, port):
        self.name = db_name
        self.db = UrlDB(db_name, set_name, host, port)  # 载入网址数据库
    ...
    def load_ini(self):
        """读取配置文件"""
        path = sys.path[0] + '\\' + self.name + '.json.bank'  # 当前文件目录下
        try:
    ...
    def add(self, url):
        ...
        # if time.time() - self.pool.get(host,
        #                                0) < self.ini['pending_threshold']:
        #     return
        host = urlparse.urlparse(url).netloc  # 解析到主机地址
        if not host or '.' not in host:  # 简单判断主机地址是否合法
            print('地址错误:', url, ', len of ur:', len(url))
            return False
        if self.db.has(host, url):
            return
        if self.pool.get(host, 0):  # 判断主机地址是否已被记录
            if self.pool[host].get(url,
                                   0)["status"] == "waiting":  # 判断链接是否已经在待下载池中
                return True
            elif time.time() - self.pool[host].get(
                    url, 0
            )["pendedtime"] < self.ini['pending_threshold']:  # 判断是否在刷新间隔中
                return
            self.waiting[host].add(url)  # 添加链接到待下载池中
            if len(self.waiting[host]) > self.max_hosts[1]:  # 判断host下的链接是否是最多的
                self.max_hosts[1] = len(self.waiting[host])  # 刷新host的最多数量
...
if __name__ == "__main__":
    # urlpool = UrlPool("demo", "test001", "127.0.0.1", "27017")
    url = "https://news.sina.com.cn/c/xl/2022-07-25/doc-imizmscv3483198.shtml"
